...
Poll the homepage without launching per-job live sampling steps. On a running detail page, keep job-level CPU time and MaxRSS from `sstat`, and sample `/proc/stat`, `/proc/meminfo`, and `/proc/loadavg` once per allocated node for node-level CPU, memory, and load curves. Derive CPU utilization from adjacent cumulative tick samples in the browser. Query GPU metrics for jobs whose Slurm TRES data indicates a GPU allocation or whose configured GPU partition provides accelerators implicitly. Use short overlapping `srun` tasks, pass both the job partition and the parent job's own account explicitly, prefix samples with the hostname, and key each series by node plus GPU index. Never rely on an implicit default account because one user may belong to multiple projects, and never run `nvidia-smi` for CPU-only jobs.
...
Explain that charts begin sampling when a job page is opened and keep recent samples in the browser; the dashboard cannot reconstruct telemetry that was never collected. Surface sampling failures as warnings without appending synthetic zero samples, and remove obsolete generic GPU placeholders once real node-qualified series arrive. The command-reference page remains copy-only.
